...
筛选对象始终是问题，不是答案。先判断问题是否属于“南哪助手新生问答&指南”应当覆盖的实用公共知识，再对纳入的问题进行聚合。本阶段不搜索、评价或生成答案，也不能使用“当前知识库是否搜得到答案”作为前置筛选条件。

初次筛选结果：

- `INCLUDE（纳入）`：属于南京大学实用问答范围，问题结合上下文后独立可读，并具有公共复用和知识维护价值。
- `AUTO_REVIEW（自动复核）`：很可能符合全部标准，但对聊天是否确实在提问或某个必要指代仍有一次复核价值；不用于收容低价值或看不懂的碎片。
- `DROP（排除）`：闲聊、娱乐、主观偏好、传闻、小道消息、通用教程、私人社交、广告、临时交易、纯情绪、Bot 命令，或者结合上下文仍无法形成独立可读问题。

口语化、错别字本身不是排除理由。模型必须利用明确上下文概括问题；必要对象、校区、年份或系统仍无法可靠还原时，直接排除而不是输出只有群友能看懂的片段。
...
- 存储目标群文本、回复关系和会话上下文。
- 将目标消息全量分块交给 LLM，按知识库问题标准完成范围初筛、独立 AI 二次审核和问题聚合，不使用关键词或问号预过滤。
- 实现回答关联和冲突保留。
- 实现脱敏及样本选择。
