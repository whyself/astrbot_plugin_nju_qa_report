...
仅靠未提供的图片、截图、视频或文件才能识别对象的问题同样属于上下文不完整；如果规范化标题本身没有
写明对象，不得根据分类或相邻候选猜测媒体内容，也不得将其猜测合并到另一条具体问题中。
最终问题复核接收报告日期，以该日期展开“今年、去年、明年、本月”等相对时间；无法定位对象范围的
“那一大堆、那几栋、这一片”等集合指代必须排除。裸域名或 URL 复制残片不承担对象含义时应从标题清理。
...
所有初筛最终纳入的问题还必须进入一次低成本最终 AI 闸门。该闸门只读取候选 ID、规范化问题、
分类、时效性和来源数量，不再读取原始聊天；它可以保留、改写、删除或合并候选。只有闸门保留的
问题才能进入回答查找和知识库调查。闸门技术错误会把相关候选标记为可重试，不得静默放行或误判为缺口。
闸门默认优先保留和规范化候选，不能因为问题粒度细、可能已有答案、标题暂未写出“南京大学”或
具有年度时效性而删除；仅对明确娱乐、无关、私人或无法识别对象的问题执行删除。
候选默认处于南京大学迎新咨询群语境，标题无需重复添加“南京大学”；陶二、南二、仙林巴士、
小百合等稳定的南大常用称呼可以直接保留，只有明确提到其他学校时才取消南大默认语境。
...
不规定固定的领域查询词。Agent 在最多 6 轮的工具调查中，根据上一轮结果自主改写查询、全文 grep、
阅读候选文档和追踪相关章节。混合检索用于召回语义相近内容，grep 用于找专有名词、系统名、文件名、
旧称和精确表述，两者互补。工具轮次结束后另有最多 2 次只允许输出最终结论的修复机会。程序只负责
仓库权限、工具参数、轮次和最终证据引用校验。
...
1. 冻结前一自然日消息窗口；
2. 校验允许仓库同步状态并创建知识快照；
3. 初筛问题，对不确定项执行 AI 自动复核，再对全部入选标题执行最终 AI 闸门；
4. 聚合纳入问题；
5. 关联群友回答；
6. 为每个问题执行独立调查；
7. 生成冻结版报告；
