...
知识库调查不是一次固定查询：对话模型会在受控循环中自行发出本地 `search`、`grep` 和 `read`
调用，根据结果改写查询并继续阅读；达到轮次上限仍不能完成时记为技术错误，不会误报知识缺口。
...
强制重跑单日必须使用 `/nju_collect report rerun YYYY-MM-DD confirm`；强制重跑全部已有历史日期
使用 `/nju_collect report rerun all confirm`。两者都只生成报告，不会自动发送邮件。
