...
- 最终问题闸门只读取初筛产生的候选标题，默认优先保留、规范化和去重，并以脱离群聊后仍能明确识别对象与信息需求为准入条件；仅靠未提供图片、截图、视频或文件才能理解的候选按上下文不完整排除；通过后才进入回答查找与知识库调查；
- 最终标题会用报告日期展开“今年”等相对时间，排除无法定位范围的集合指代，并清理不承担对象含义的裸域名或 URL 残片；
...
- 语雀批准/排除策略、正文增量同步与分块；知识调查 Agent 会自行改写混合检索查询、执行全文 grep，并继续阅读命中文档后再判定覆盖状态；
...
带 `confirm` 的 `report rerun` 才会重新调用指定日期的 AI 筛选和知识调查；使用
`report rerun all confirm` 会逐日强制重跑全部已有聊天记录的历史日期。长任务不会主动
