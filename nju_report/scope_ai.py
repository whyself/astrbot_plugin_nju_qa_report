...
而应优先保留、规范化和去重。但“脱离原聊天后语义完整、读者能独立看懂”是每个最终问题的
首要准入条件，优先级高于“尽量保留”。最终标题必须同时说清楚：在问什么对象，以及想了解该对象的
什么客观信息。只有明确属于娱乐审美、口味推荐、通用软件教程、私人社交、传闻闲聊，或仅凭候选文字
无法识别对象或信息需求的问题才删除。不得因为问题细、答案可能已存在、问题可能只有一句话、
标题没写“南京大学”，或答案具有年度时效性而删除。

所有候选都来自南京大学迎新咨询群，默认语境就是南京大学。标题不需要重复添加“南京大学”，
也不能因缺少这四个字而降低清晰度或知识价值。陶二、南二、仙林巴士、小百合等群内稳定使用的
南大常用称呼可以直接保留，不要求展开全称。只有候选明确提到其他学校时，才不得套用南大默认语境。
“这个、后面那栋、28-30”等没有稳定对象的临时指代仍不能猜测。常用简称本身可以是明确对象；
纯数字、代词、方位词或只在上一句话中才有含义的省略表达不是明确对象。图片、截图、表情、视频、
文件等媒体内容也没有提供给本阶段；“图中、图片里、截图上、视频中、这个文件”等媒体指代不算
明确对象，不能根据分类、相邻候选或常识猜测媒体内容。

每个候选都附有 report_date，表示聊天所属自然日，也是“今年、去年、明年、这个月”等相对时间的
唯一换算基准。最终标题不得保留会随阅读日期变化的相对时间：在不添加事实的前提下换成明确年份或
月份。例如 report_date 为 2026-07-12 时，“今年通知书会不会晚一点”应改写为
“2026年南京大学录取通知书发放时间是否较往年延迟”。不得从知识库文档年份猜测“今年”。

逐项执行最终编辑：
- 保留：问题已经实用、南大相关、公共可复用且独立可读；
- 改写：保持南大默认语境和常用简称，去掉情绪、传闻前提和概率口吻，
...
输出前对每个 canonical_question 单独做一次“陌生维护者测试”：假设维护者只能看到这一行，
完全看不到群聊上下文，他是否能唯一理解问题对象和待补充的知识点？能则保留；不能则仅在候选文字
已经提供足够信息时改写补全，不能从候选文字可靠补全就省略该项。不要把语义不清的问题交给后续
知识库调查，也不要指望调查阶段反推原聊天上下文。

硬性校验示例：
- “录取通知书有什么独家设计”“哪些窗口好吃”“WPS 与 Office 混用教程”必须删除；
- “28-30什么时候好”只有数字范围和模糊的“好”，对象与信息需求都不完整，必须删除；
- “南三后面那栋是否翻新”中的“后面那栋”无法仅凭标题唯一定位，必须删除；
- “鼓楼那一大堆什么时候翻完”“那几栋什么时候装修好”中的楼栋范围无法定位，必须删除；
- “那一大堆、那几栋、这一片、剩下那些、这些宿舍”等集合指代，只有候选文字本身已经明确列出
  对象或范围时才能改写保留，不能用“鼓楼、仙林”等大范围地点代替具体对象；
- “图片中的建筑是什么”“截图里的这个页面怎么用”依赖未提供的媒体，必须作为上下文不完整删除；
- 如果候选自身已经写明对象，如“图片中的南京大学北大楼是什么建筑风格”，可去掉图片前提后改写为
  “南京大学北大楼是什么建筑风格”；关键是对象和信息需求必须由候选文字本身明确给出；
- 不得仅因另一条具体候选与媒体问题出现在同一批、分类相同或看起来相关，就把媒体问题猜测合并进去；
- 标题中的裸域名、URL 和复制残片若不承担对象含义，应删除后再判断核心问题；例如
  “.nju.edu.cn 专业选修课是啥”应改写为“南京大学专业选修课是什么”。如果删除残片后没有明确问题，
  则删除整个候选；
- “能否有机会住新宿舍”只有能在不添加事实的前提下改成明确的宿舍分配对象或规则问题时才保留，
  否则删除；
- “陶二宿舍条件如何”应保留并改写为“陶二宿舍的房型和设施配置如何”；
- “鼓楼宿舍是否配备马桶”是可核实的客观设施问题，必须保留；不能因为粒度较细而删除；
- “小百合是什么”必须保留；“小百合关闭后由什么校内平台接替”也必须保留；
  两者都是南大校园信息平台问题，不是娱乐趣闻；
- “专业选修课是什么”必须保留；南大课程体系是默认语境，无需在标题中重复写出；
        ...
        payload_candidates.append(
            {
                ...
                "report_date": str(item.report_date).strip(),
            }
        )
